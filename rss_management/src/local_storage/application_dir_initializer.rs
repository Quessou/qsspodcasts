        ...
        Path::new(&path).exists()
            && Path::new(
                &(path.to_str().unwrap().to_owned()
                    + "/"
                    + self.path_provider.rss_feed_list_file_name()),
            )
            .exists()
            && Path::new(
                &(path.to_str().unwrap().to_owned() + "/" + self.path_provider.download_dir_name()),
            )
            .exists()
    }
    ...
    fn is_path_valid(path: &Path) -> Result<(), std::io::Error> {
        if path.exists() && path.is_file() {
            ...
            return Err(std::io::Error::new(
                ...
                "Path points out to an already existing file",
            ));
        }

        if path.is_relative() {
            warn!(
                "Path is relative, which may lead to unexpected behavior : {}",
                path.to_str().unwrap()
            );
            return Err(std::io::Error::new(
...
mod tests {
    ...
    fn test_path_integrity_file_already_exists() -> Result<(), String> {
        ...
        assert_eq!(
            ApplicationDirInitializer::is_path_valid(&file_path)
                .unwrap_err()
                .kind(),
            ErrorKind::AlreadyExists
        );
    ...
    fn test_path_integrity_relative_path() -> Result<(), String> {
        ...
        assert_eq!(
            ApplicationDirInitializer::is_path_valid(&relative_file_path)
                .unwrap_err()
                .kind(),
            ErrorKind::InvalidInput
        );
    ...
    fn test_path_integrity_permissions() -> Result<(), String> {
        ...
        assert_eq!(
            test_dir_path.metadata().unwrap().permissions().mode() & 0xFFF,
            0o600
        );
        assert_eq!(
            ApplicationDirInitializer::is_path_valid(&test_dir_path)
                .unwrap_err()
                .kind(),
            ErrorKind::PermissionDenied
        );

        fs::set_permissions(&test_dir_path, fs::Permissions::from_mode(0o700))
            .expect("Initialization of test failed");
